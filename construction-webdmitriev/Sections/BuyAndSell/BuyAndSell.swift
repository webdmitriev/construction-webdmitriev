...
final class BuyAndSell {
    ...
    func createSection() -> NSCollectionLayoutSection {
        ...
        let groupSize = NSCollectionLayoutSize(widthDimension: .absolute(191), heightDimension: .absolute(135))
        let group = NSCollectionLayoutGroup.horizontal(layoutSize: groupSize, subitems: [item])
        group.contentInsets = .init(top: 0, leading: 0, bottom: 0, trailing: 8)
        
        let section = NSCollectionLayoutSection(group: group)
        section.orthogonalScrollingBehavior = .continuous
        section.contentInsets = .init(top: 0, leading: insert, bottom: 0, trailing: 0)
